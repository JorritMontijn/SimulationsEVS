...
function sData = getSimRunDistrib_xArea(sData,dblMaxRunningTime,intWorker)
	...
	while toc(hTic) < dblMaxRunningTime && intRepCounter <= intMaxRep
		...
		if intIter > 0
			...
			%build stimulus vectors for next repetition
			vecRandStimTypes = randperm(numel(vecTrialStimTypeIn));
			
			vecTrialStimType = cat(2,vecTrialStimType,vecRandStimTypes);
			vecTrialStimRep = cat(2,vecTrialStimRep,vecTrialStimRepIn*intRepCounter);
		...
		for dblCurT = vecOverallRepT
			...
			if boolStimPresent && intTrialT < size(cellLGN_ON{intStimType},3)
