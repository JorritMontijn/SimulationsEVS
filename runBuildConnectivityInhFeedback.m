...
%synaptic delays
sConnParams.dblDelayMeanV1ToV2 = 5/1000; %in ms
sConnParams.dblDelaySDV1ToV2 = 3/1000; %in ms

%V2=>V1
dblInterArealFeedbackFactor = 1;
sConnParams.dblSpatialDropoffV2V1 = 0.8; %normpdf(vecX,0,0.8); zandvakili&kohn, 2015
sConnParams.matConnsPerTypeV2V1(1,:) = [0 0];%from pyramid to [pyr inter]
sConnParams.matConnsPerTypeV2V1(2,:) = [24 24];%from interneuron to [pyr inter]
...
%synaptic delays
sConnParams.dblDelayMeanV2ToV1 = 5/1000; %in ms
sConnParams.dblDelaySDV2ToV1 = 3/1000; %in ms

%% build connectivity
sConnectivity = buildConnectivity_xArea(sConnParams);

%% save
strConnFile = sprintf('sConnInhFB_Col%dN%dS%d_%s.mat',intColumns,sConnectivity.intCortexCells,numel(sConnectivity.vecSynExcInh),getDate);
