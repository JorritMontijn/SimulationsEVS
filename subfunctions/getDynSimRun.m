...
function sData = getDynSimRun(sData,dblMaxRunningTime,intWorker)
	%% prep
	...
	while toc(hTic) < dblMaxRunningTime && intRepCounter < intMaxRep
		...
		for dblCurT = vecOverallRepT
			...
			if boolStimPresent && intTrialT < size(matLGN_ON,3)
				...
				matThisIterON = matLGN_ON(:,:,intTrialT)>matRandON;
				matThisIterOFF = matLGN_OFF(:,:,intTrialT)>matRandOFF;
			else
			...
			for intField=[1 2]
				...
				for intNeuron=find(vecRSCounterLGN>0 & ~indMoot)
					...
					%calculate PSPs
					vecPSPs(vecTargetSynapses) = vecSW(vecTargetSynapses) .* vecSC(vecTargetSynapses) .* fPSP(dblCurT-dblDelay,vecSpikeTimes,dblTauPeak);
				end
