	...
	if intCellsV2>0
		...
		parfor intCellV1=1:intCellsV1
			...
			for intCellV2=1:intCellsV2
				...
				b = b - (sum(b(:),'double') / numel(b));
				r = sum(sum(a.*b))/sqrt(sum(sum(a.*a))*sum(sum(b.*b)));
				matSimilarityGaborExemplar(intCellV1,intCellV2) = r;
			end
		end
		%msg
		...
		for intCellV2=1:intCellsV2
			...
			if intTargetCellType==1
				%exemplar-based
				vecTuningSimilarity = matSimilarityGaborExemplar(:,intCellV2);
			...
			if boolUseWeights
				...
				vecSynV1V2Weight(vecConnsAssign) = ones(size(vecTheseWeights));
			end
			...
			matFieldsV2(:,:,intCellV2) = xmean(bsxfun(@mtimes,matPrefGabors(:,:,vecConns),reshape(vecSynV1V2Weight(vecConnsAssign),[1 1 intConnsToThisType])),3);
			%{
	subplot(2,2,1);
