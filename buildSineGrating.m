function [matStim,rampGrid] = buildSineGrating(SpatFreq,StimSizeRetDeg,ScrPixWidthHeight,ScrDegWidthHeight,phaseOffset)
	%buildGrating outputs greyscale grating patch
	%   syntax: matStim = buildSineGrating(SpatFreq,StimSizeRetDeg,ScrPixWidthHeight,ScrDegWidthHeight,phaseOffset)
	...
	%build the grating
	modmat = mod(grid-1+pixPerCycle*(phaseOffset/(2*pi)),pixPerCycle);  %every pixPerCycle pixels the grid flips back to 0 with an offset of phaseOffset
	matGrat = (((sin((modmat/255)*2*pi))+1)/2)*255; %set range [0-255]
