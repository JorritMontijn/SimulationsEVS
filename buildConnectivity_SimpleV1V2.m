function sConnectivity = buildConnectivity_SimpleV1V2(sConnParams)
	...
	vecConnProbSD = sConnParams.vecConnProbSD; %degrees difference in pref ori, for [pyramid interneuron]
	...
	if intCellsV2>0
		...
		%copy connections from V1-V1 to V1-V2
		matFieldsV2 = matPrefGabors;
		indCopyConns = vecSynV1ExcInh==1;
		matSynV1V2FromTo = matSynV1FromTo(indCopyConns,:);
		matSynV1V2FromTo(:,2) = matSynV1V2FromTo(:,2) + intCellsV1;
		intTotalV1V2Connections = size(matSynV1V2FromTo,1);
		vecSynV1V2Conductance = nan(intTotalV1V2Connections,1); %synaptic conductance
		vecSynV1V2Weight = ones(intTotalV1V2Connections,1); %synaptic conductance
		vecSynV1V2Delay = max(0,gaussrnd(dblDelayMeanV1ToV2,dblDelaySDV1ToV2,[intTotalV1V2Connections 1])); %synaptic delay
		vecSynV1V2ExcInh = ones(intTotalV1V2Connections,1); %only pyramids project
		vecSynV1V2Type = 2*ones(intTotalV1V2Connections,1); %1; intra-areal, 2 inter-areal
		...
		for intCellV2=1:intCellsV2
			...
			indConnsAssign = matSynV1V2FromTo(:,2) == intNeuron;
			...
			vecSynV1V2Conductance(indConnsAssign) = dblG; %synaptic conductance
		...
		for intCellV2=1:intCellsV2
			...
			for intSubIdx=1:length(vecCellTypeLabels)
				...
				vecConnProb(~indThisType) = 0; %remove other types
				vecConnProb(intCellV2) = 0; %remove the neuron itself
				vecConnProb = vecConnProb/sum(vecConnProb); %normalize so integral is 1
				vecProbCumSum = cumsum(vecConnProb);
